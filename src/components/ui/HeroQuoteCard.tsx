...
export const HeroQuoteCard = React.forwardRef<HTMLDivElement, HeroQuoteCardProps>(
 ...
 <p className={cn("font-serif text-[1.55rem] leading-[1.34] tracking-tight max-w-[34ch] relative", description ? "mb-4" : "", variant === 'green' ? 'text-[1.38rem] leading-[1.4]' : '')}>
 ...
 <p className={cn("font-serif text-[1.55rem] leading-[1.34] tracking-tight max-w-[38ch] relative", description ? "mb-4" : "mb-10", variant === 'green' ? 'text-[1.38rem] leading-[1.4]' : '')}>
                {showQuotes ? `"${quote}"` : quote}
              </p>
              {description && (
